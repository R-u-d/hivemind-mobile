...
def generate_cover_presigned_url(community_id: str, content_type: str, file_size: int) -> dict:
    ...
    if "/" in community_id_str or ".." in community_id_str:
        raise S3Error("Invalid community identifier.")

    key = f"covers/{community_id_str}/{uuid.uuid4()}.{ext}"
    ...
    try:
        ...
        logger.exception("Failed to generate cover presigned URL")
        raise S3Error("Could not generate upload URL.") from exc
